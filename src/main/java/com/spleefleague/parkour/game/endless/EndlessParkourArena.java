...
public class EndlessParkourArena extends Arena<EndlessParkourBattle> {
        ...
        disableTask = new BukkitRunnable() {
            public void run() {
                ...
                if(min < 5) {
                    if(!disabled) {
                        ChatManager.sendMessage(Parkour.getInstance().getChatPrefix(), Theme.SUCCESS.buildTheme(false)
                                + ChatColor.GREEN + "Endless has been disabled for scheduled maintenance (11:55pm - 12:00am PST)"
                            ...
                            if(battle != null) {
                                battle.getActivePlayers().forEach(sjp -> {
                                    ChatManager.sendMessagePlayer(SpleefLeague.getInstance().getPlayerManager().get(sjp.getPlayer())
                                            ...
                                            + ChatColor.RED + " Your battle was ended for scheduled maintenance.");
                                });
                    ...
                    ChatManager.sendMessage(Parkour.getInstance().getChatPrefix(), Theme.SUCCESS.buildTheme(false)
                            + ChatColor.GREEN + "Endless is now enabled, thank you for your patience"
                            , Parkour.getInstance().getStartMessageChannel());
                    disabled = false;
                }
                ...
                if(sec == 0) {
                    if(min == 15) {
                        ChatManager.sendMessage(Parkour.getInstance().getChatPrefix(), Theme.SUCCESS.buildTheme(false)
                                + ChatColor.GREEN + "Endless will be disabled in "
                                + ChatColor.RED + "10"
                                + ChatColor.GREEN + " minutes for scheduled maintenance"
                                , Parkour.getInstance().getStartMessageChannel());
                    }
                    else if(min == 10) {
                        ChatManager.sendMessage(Parkour.getInstance().getChatPrefix(), Theme.SUCCESS.buildTheme(false)
                        ...
                        ChatManager.sendMessage(Parkour.getInstance().getChatPrefix(), Theme.SUCCESS.buildTheme(false)
                                + ChatColor.GREEN + "Endless will be disabled in "
                                + ChatColor.RED + "1"
                                + ChatColor.GREEN + " minute for scheduled maintenance"
                                , Parkour.getInstance().getStartMessageChannel());
                    }
                }
            }
    ...
    public void checkHighscore(ParkourPlayer sjp) {
        ...
        try {
            EntityBuilder.save(highscoreMap.get(highscoreDate), Parkour.getInstance().getPluginDB().getCollection("ArenaScores"));
        } catch(Exception e) {
    ...
    private static int frequencySumB, frequencySumF, frequencySumI;
    private static ArrayList<Jump> possibleJumpsB, possibleJumpsF, possibleJumpsI;
    ...
    private static ArenaData generate(Location spawn1, int jumpCount, int level) {
        ...
        if(level <= 200) {
            ...
            difficulty = Math.max(3.f + (float)floor((level - 200) / 10.f) * 0.2f, 4.f);
        }
        possibleJumpsB = ParkourRandomJumps.getJumpsByDifficultyB(difficulty);
        possibleJumpsF = ParkourRandomJumps.getJumpsByDifficultyF(difficulty);
        possibleJumpsI = ParkourRandomJumps.getJumpsByDifficultyI(difficulty);
        frequencySumB = 0;
        for(Jump j : possibleJumpsB) {
            frequencySumB += j.getFrequency(difficulty);
        }
        frequencySumF = 0;
        for(Jump j : possibleJumpsF) {
            frequencySumF += j.getFrequency(difficulty);
        }
        frequencySumI = 0;
        for(Jump j : possibleJumpsI) {
            frequencySumI += j.getFrequency(difficulty);
        }
        ...
        for (int j = 0; j < jumpCount; j++) {
            switch(last) {
                case 'b':
                    ...
                    break;
                case 'f':
                    next = getNextJump(possibleJumpsF, frequencySumF, difficulty);
                    break;
                case 'i':
                default:
                    next = getNextJump(possibleJumpsI, frequencySumI, difficulty);
                    break;
            }
            ...
            if (j < jumpCount - 1) {
                switch(next.getTo()) {
                    case 'b':
                        ...
                        break;
                    case 'f':
                        fakeBlocks.add(new BlockPrototype(lastLoc.clone(), Material.FENCE, (byte)0));
                        break;
                    case 'i':
                    default:
                        fakeBlocks.add(new BlockPrototype(lastLoc.clone(), Material.IRON_FENCE, (byte)0));
                        break;
                }
            } else {
                goal = lastLoc;
